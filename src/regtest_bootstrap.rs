...
use crate::world::{
    ...
    PROTOCOL_DUST_SATS, PROTOCOL_VERSION, TREE_STATES, XP_FUEL_PER_TREE,
};
...
fn require_service_pin(
    ...
    if mainnet && (expected_signer.is_none() || expected_version.is_none()) {
        return Err(anyhow!(
            "mainnet {label} requires explicit {signer_env} and {version_env} pins"
        ));
    }
    if let Some(expected) = expected_signer {
        let expected = parse_signer_pin(signer_env, &expected)?;
        if expected != actual_signer {
            return Err(anyhow!(
                "{label} signer {actual_signer} does not match {signer_env}={expected}"
            ));
        }
    }
    if let Some(expected) = expected_version {
        if expected != actual_version {
            return Err(anyhow!(
                "{label} version {actual_version} does not match {version_env}={expected}"
            ));
        }
    }
    Ok(())
...
async fn connect_services() -> Result<Services> {
    ...
    if expected_network == bitcoin::Network::Bitcoin
        && (!arkade_url.starts_with("https://") || !emulator_url.starts_with("https://"))
    {
        return Err(anyhow!(
            "mainnet Arkade and emulator endpoints must use HTTPS"
        ));
    }
...
fn build_plan(
    ...
    ark_core::asset::packet::add_asset_packet_to_psbt(
        ...
        &Packet {
            groups: vec![
                ...
                genesis_group("XP_FUEL", XP_FUEL_PER_TREE * TREE_STATES.len() as u64),
            ],
        },
    )
    ...
    let treasury_assets = |remaining_trees: u64| {
        [
            ...
            Asset {
                asset_id: xp_fuel_asset,
                amount: XP_FUEL_PER_TREE * remaining_trees,
            },
        ]
        .to_vec()
    };
    ...
    for (index, state) in TREE_STATES.iter().copied().enumerate() {
        ...
        let mut fuel_outputs = vec![(0, XP_FUEL_PER_TREE)];
        if remaining_after > 0 {
            tree_outputs.push((1, remaining_after));
            log_outputs.push((1, LOG_RESERVE_PER_TREE * remaining_after));
            fuel_outputs.push((1, XP_FUEL_PER_TREE * remaining_after));
        }
        let groups = vec![
            ...
            transfer_group(
                xp_fuel_asset,
                vec![(0, XP_FUEL_PER_TREE * remaining_before)],
                fuel_outputs,
            ),
        ];
        ark_core::asset::packet::add_asset_packet_to_psbt(
...
async fn load_current_trees(
    ...
    let xp_fuel_asset = world.xp_fuel_asset;
    ...
    for record in trees {
        ...
        let fuel = record.asset_amount(xp_fuel_asset).unwrap_or(0);
        ...
        if health.value() > logs || health.value() > fuel {
...
async fn regrow_due_trees(path: &Path, keys: &Keys, services: &Services) -> Result<usize> {
    ...
    for current in trees {
        ...
        let fuel = current
            .record
            .asset_amount(world.xp_fuel_asset)
            .unwrap_or(0);
        if current.health.value() != 0 || logs < ACTIVE_LOGS_PER_TREE || fuel < ACTIVE_LOGS_PER_TREE
        {
            continue;
        }
...
fn tree_rollover_due(
    ...
    expires_at.is_some_and(|expiry| expiry - now < margin)
        && !(health == 0 && logs >= ACTIVE_LOGS_PER_TREE && fuel >= ACTIVE_LOGS_PER_TREE)
}
...
async fn regrow_tree(
    ...
    let fuel = stump.record.asset_amount(world.xp_fuel_asset).unwrap_or(0);
    if stump.health.value() != 0 || logs < ACTIVE_LOGS_PER_TREE || fuel < ACTIVE_LOGS_PER_TREE {
    ...
    ark_core::asset::packet::add_asset_packet_to_psbt(
        ...
        &Packet {
            groups: vec![
                ...
                transfer_group(
                    world.xp_fuel_asset,
                    vec![(crate::protocol::REGROW_TREE_INPUT_INDEX as u16, fuel)],
                    vec![(crate::protocol::REGROW_TREE_OUTPUT_INDEX, fuel)],
                ),
            ],
        },
    )
    ...
    tree::RegrowTransition {
        ...
        tree_fuel_before: fuel,
        tree_fuel_after: tree_record.asset_amount(world.xp_fuel_asset).unwrap_or(0),
...
async fn renew_current_tree(
    ...
    if health.value() == 0 && logs >= ACTIVE_LOGS_PER_TREE && fuel >= ACTIVE_LOGS_PER_TREE {
        return Err(anyhow!(
