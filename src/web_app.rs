...
impl ChopMutation {
    fn parse(name: &str) -> Result<Self> {
        match name {
            ...
            "wrong-fuel-delta" => Ok(Self::WrongFuelDelta),
...
impl WoodlandApp {
            ...
            Some(json) => {
                ...
                if let Some(asset) = profile.player_asset.as_deref() {
                    let asset = asset
                        ...
                        .map_err(js_err)?;
                    if [tree_asset, log_asset, xp_fuel_asset].contains(&asset) {
                        return Err(JsValue::from_str(
    ...
    #[cfg(feature = "regtest-e2e")]
    #[wasm_bindgen(js_name = testInvalidFuelGroup)]
    pub async fn test_invalid_fuel_group(&mut self, tree_id: u32) -> Result<(), JsValue> {
        self.invalid_fuel_group_probe(tree_id).await.map_err(js_err)
    }

    #[cfg(feature = "regtest-e2e")]
    #[wasm_bindgen(js_name = testInvalidLogFuelGroupOrder)]
    pub async fn test_invalid_log_fuel_group_order(&mut self, tree_id: u32) -> Result<(), JsValue> {
        self.invalid_log_fuel_group_order_probe(tree_id)
            .await
            .map_err(js_err)
    }
...
impl WoodlandApp {
    fn snapshot(&self) -> HarnessSnapshot {
        ...
        let trees = self
            ...
            .map(|tree| {
                ...
                let fuel = tree
                    .record
                    .asset_amount(self.world.xp_fuel_asset)
                    .unwrap_or(0);
                let (next_roll, raw_drop) = tree.roll.advance(player_xp);
                let next_drop = raw_drop && tree.health.value() > 0 && logs > 0 && fuel > 0;
                let can_regrow = tree.health.value() == 0
                    && logs >= self.world.manifest.active_logs_per_tree
                    && fuel >= self.world.manifest.active_logs_per_tree;
                TreeView {
        ...
        HarnessSnapshot {
            ...
            xp_fuel_asset: self.world.xp_fuel_asset.to_string(),
    ...
    async fn sync(&mut self) -> Result<()> {
        ...
        self.player_state = match player_state_record {
            Some(record) => {
                ...
                let fuel = record.asset_amount(self.world.xp_fuel_asset).unwrap_or(0);
                if state.identity != expected_identity || state.xp.value() != fuel {
                    return Err(anyhow!(
                        "current player state has invalid identity or XP_FUEL backing"
                    ));
                }
    ...
    async fn invalid_fuel_group_probe(&mut self, tree_id: u32) -> Result<()> {
        self.rejected_chop_mutation_probe(tree_id, ChopMutation::ReplaceFuelGroup)
            .await
    }

    async fn invalid_log_fuel_group_order_probe(&mut self, tree_id: u32) -> Result<()> {
        self.rejected_chop_mutation_probe(tree_id, ChopMutation::SwapLogFuelGroups)
            .await
    }
    ...
    async fn chop_inner_with_options(
        ...
        let player_fuel_before = state
            .record
            .asset_amount(self.world.xp_fuel_asset)
            .unwrap_or(0);
        if state.state.xp.value() != player_fuel_before {
            return Err(anyhow!("player XP is not backed by XP_FUEL"));
        }
        ...
        let tree_fuel_before = require_nonzero_asset(
            &tree.record,
            self.world.xp_fuel_asset,
            "tree has no XP_FUEL",
        )?;
        ...
        let fuel_reward = if matches!(mutation, ChopMutation::WrongFuelDelta) {
            1 - reward
        } else {
            reward
        };
        ...
        let tree_fuel_after = tree_fuel_before - fuel_reward;
        let player_fuel_after = player_fuel_before
            .checked_add(fuel_reward)
        ...
        let mut fuel_inputs = Vec::new();
        if player_fuel_before > 0 {
            fuel_inputs.push((
                crate::protocol::PLAYER_STATE_INPUT_INDEX as u16,
                player_fuel_before,
            ));
        }
        fuel_inputs.push((crate::protocol::TREE_INPUT_INDEX as u16, tree_fuel_before));
        let mut fuel_outputs = Vec::new();
        if player_fuel_after > 0 {
            fuel_outputs.push((
                crate::protocol::PLAYER_STATE_OUTPUT_INDEX,
                player_fuel_after,
            ));
        }
        if tree_fuel_after > 0 {
            fuel_outputs.push((crate::protocol::TREE_OUTPUT_INDEX, tree_fuel_after));
        }
        let mut groups = vec![
            transfer_group(
                player_asset,
            ...
            transfer_group(self.world.xp_fuel_asset, fuel_inputs, fuel_outputs),
        ];
        ...
        if matches!(mutation, ChopMutation::SwapLogFuelGroups) {
            groups.swap(
                crate::protocol::LOG_ASSET_GROUP_INDEX,
                crate::protocol::XP_FUEL_ASSET_GROUP_INDEX,
            );
        }
        if matches!(mutation, ChopMutation::ReplaceFuelGroup) {
            groups[crate::protocol::XP_FUEL_ASSET_GROUP_INDEX].asset_id = Some(AssetId {
        ...
        PlayerChopTransition {
            ...
            state_fuel_before: player_fuel_before,
            state_fuel_after: state_record
                .asset_amount(self.world.xp_fuel_asset)
                .unwrap_or(0),
...
fn select_tree_records(
    ...
    for record in records {
        ...
        let log_entries = record
            ...
            .count();
        let fuel_entries = record
            .assets
            .iter()
            .filter(|asset| asset.asset_id == world.xp_fuel_asset)
            .count();
        let logs = record.asset_amount(world.log_asset).unwrap_or(0);
        let fuel = record.asset_amount(world.xp_fuel_asset).unwrap_or(0);
