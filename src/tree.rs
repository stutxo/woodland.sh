...
//! Successful chops reduce both health and the remaining LOG/XP_FUEL inventory;
...
use crate::protocol::{
    ...
    TREE_STATE_PACKET_TYPE, XP_FUEL_ASSET_GROUP_INDEX,
};
...
/// Mutable health, LOG reserve, and XP_FUEL inventory are committed separately.
...
pub struct ChopTransition {
    ...
    pub tree_fuel_before: u64,
    pub tree_fuel_after: u64,
    pub player_logs_before: u64,
    pub player_logs_after: u64,
    pub player_fuel_before: u64,
    pub player_fuel_after: u64,
...
impl ChopTransition {
    pub fn validate(self) -> Result<()> {
        ...
        if self.state_xp_before != self.player_xp_before
            || self.state_xp_before != self.player_fuel_before
            || self.state_xp_after != self.player_fuel_after
            || self.state_xp_after
                != self
                    .state_xp_before
                    .checked_add(reward)
        ...
        {
            return Err(anyhow!(
                "player XP counter does not match conserved XP_FUEL"
            ));
        }
        ...
        if self.tree_fuel_after
            != self
                .tree_fuel_before
                .checked_sub(reward)
                .ok_or_else(|| anyhow!("tree XP_FUEL balance underflow"))?
            || self.player_fuel_after
                != self
                    .player_fuel_before
                    .checked_add(reward)
                    .ok_or_else(|| anyhow!("player XP_FUEL balance overflow"))?
        {
            return Err(anyhow!("XP_FUEL balances do not match the chop roll"));
        }
        if self.dust_sats == 0
...
impl RegrowTransition {
    pub fn validate(self) -> Result<()> {
        ...
        if self.tree_logs_before != self.tree_logs_after
            || self.tree_fuel_before != self.tree_fuel_after
            || self.tree_logs_before < LOGS_PER_TREE
            || self.tree_fuel_before < LOGS_PER_TREE
        {
            return Err(anyhow!(
                "regrowth must preserve at least {LOGS_PER_TREE} LOG and XP_FUEL"
            ));
        }
...
pub fn build_tree_contract<C: Verification>(
    ...
    if [tree_asset, log_asset, xp_fuel_asset]
        .into_iter()
        .collect::<std::collections::HashSet<_>>()
        .len()
        != 3
    {
        return Err(anyhow!("TREE, LOG, and XP_FUEL asset IDs must differ"));
    }
    ...
    let chop_arkade_script = tree_covenant_script(tree_asset, log_asset, xp_fuel_asset, dust_sats)?;
    let regrow_arkade_script =
        tree_regrow_covenant_script(tree_asset, log_asset, xp_fuel_asset, dust_sats)?;
    let renewal_arkade_script = tree_renewal_covenant_script(tree_asset, log_asset, xp_fuel_asset)?;
...
///
/// The tree transfers LOG and XP_FUEL into owner-authorized player state and
/// requires numeric XP to equal the player's conserved XP_FUEL balance.
///
/// Canonical shape:
///
/// ```text
...
/// groups 0..3 PLAYER_ID | TREE | LOG | XP_FUEL
/// ```
///
...
pub fn tree_covenant_script(
    ...
    for (asset, group_index) in [
        ...
        (xp_fuel_asset, XP_FUEL_ASSET_GROUP_INDEX as i64),
    ] {
        builder = push_asset_group_index(builder, asset)
            .push_int(group_index)
            .push_opcode(OP_EQUALVERIFY);
    }
    for asset in [tree_asset, log_asset, xp_fuel_asset] {
    ...
    let builder = push_canonical_player_asset_counts(builder, log_asset, xp_fuel_asset);
    ...
    // XP_FUEL is conserved and moved into player state, where it backs XP.
    let builder = push_input_asset_lookup(builder, TREE_INPUT_INDEX, xp_fuel_asset)
        .push_int(1)
        .push_opcode(OP_EQUALVERIFY)
        .push_opcode(OP_DUP)
        .push_int(0)
        .push_opcode(OP_GREATERTHAN)
        .push_opcode(OP_VERIFY);
    let builder = push_optional_output_asset_lookup(builder, TREE_OUTPUT_INDEX, xp_fuel_asset)
        .push_opcode(OP_DROP);
    let builder = push_advanced_tree_roll(builder, TREE_INPUT_INDEX, PLAYER_STATE_INPUT_INDEX)
        .push_opcode(OP_ADD)
        .push_opcode(OP_EQUALVERIFY);
    let builder =
        push_optional_input_asset_lookup(builder, PLAYER_STATE_INPUT_INDEX, xp_fuel_asset)
            .push_opcode(OP_DROP);
    let builder = crate::player::push_player_input_xp(builder, PLAYER_STATE_INPUT_INDEX)
        .push_opcode(OP_EQUALVERIFY);
    let builder =
        push_optional_output_asset_lookup(builder, PLAYER_STATE_OUTPUT_INDEX, xp_fuel_asset)
            .push_opcode(OP_DROP);
...
///
/// A stump keeps one TREE marker, its remaining fixed-supply LOG and XP_FUEL,
...
pub fn tree_regrow_covenant_script(
    ...
    let builder = push_canonical_asset_group(builder, xp_fuel_asset, 1, 1);
    ...
    let builder = push_local_asset_group_inputs(builder, xp_fuel_asset, 1);
    ...
    let builder = push_input_asset_lookup(builder, REGROW_TREE_INPUT_INDEX, xp_fuel_asset)
        .push_int(1)
        .push_opcode(OP_EQUALVERIFY)
        .push_opcode(OP_DUP)
        .push_int((LOGS_PER_TREE - 1) as i64)
        ...
        .push_opcode(OP_VERIFY);
    Ok(
        push_output_asset_lookup(builder, REGROW_TREE_OUTPUT_INDEX, xp_fuel_asset)
            .push_int(1)
            .push_opcode(OP_EQUALVERIFY)
            .push_opcode(OP_EQUAL)
            .into_script(),
...
/// proof and only permits an exact self-send: identical P2TR, value,
/// TREE/LOG/XP_FUEL assets, and tree-state packets. Renewal changes nothing; it
/// only re-enters the VTXO into a fresh batch for a new expiry.
///
...
pub fn tree_renewal_covenant_script(
    ...
    let builder = push_optional_transfer_group_shell(builder, xp_fuel_asset);
    ...
    let builder =
        push_optional_input_asset_lookup(builder, RENEWAL_STATE_INPUT_INDEX, xp_fuel_asset);
    let builder =
        push_optional_output_asset_lookup(builder, RENEWAL_STATE_OUTPUT_INDEX, xp_fuel_asset);
    Ok(builder
        .push_opcode(OP_ROT)
        .push_opcode(OP_EQUALVERIFY)
        .push_opcode(OP_EQUAL)
...
/// Require player state to contain its mandatory PLAYER_ID plus at most one
/// LOG and one XP_FUEL holding. Both reciprocal chop covenants use this exact
/// sequence so their view of canonical player inventory cannot drift.
...
pub(crate) fn push_player_renewal_group_set(
    ...
    let builder = push_optional_transfer_group_shell_counted(builder.push_int(1), log_asset)
        .push_opcode(OP_ADD);
    push_optional_transfer_group_shell_counted(builder, xp_fuel_asset)
        .push_opcode(OP_ADD)
        .push_opcode(op::INSPECTNUMASSETGROUPS)
        .push_opcode(OP_EQUALVERIFY)
}
...
mod tests {
    ...
    #[test]
    fn successful_chop_moves_fuel_and_log_to_player() {
        ...
        let mut unburned_fuel = valid;
        unburned_fuel.tree_fuel_after = 5;
        assert!(unburned_fuel.validate().is_err());
        let mut overburned_fuel = valid;
        overburned_fuel.tree_fuel_after = 3;
        assert!(overburned_fuel.validate().is_err());
    ...
    fn missed_chop_advances_roll_without_burning_inventory() {
        ...
        let mut burned_fuel = valid;
        burned_fuel.tree_fuel_after = 4;
        assert!(burned_fuel.validate().is_err());
    }
    ...
    fn regrowth_resets_health_without_issuing_assets() {
        ...
        low_reserve.tree_fuel_before = 4;
    ...
    fn many_permissionless_players_preserve_world_supply_across_many_schedules() {
        ...
        for seed in 1_u64..=32 {
            let mut players_xp = [0_u64; 256];
            let mut players_logs = [0_u64; 256];
            let mut players_fuel = [0_u64; 256];
            ...
            loop {
                for tree in &mut trees {
                    if tree.health.value() == 0 && tree.logs >= LOGS_PER_TREE && !tree.regrown {
                        RegrowTransition {
                            ...
                            tree_fuel_before: tree.fuel,
                            tree_fuel_after: tree.fuel,
                            tree_value_before: 1_980,
                            tree_value_after: 1_980,
                        }
                        .validate()
                ...
                entropy = entropy
                    .wrapping_mul(6_364_136_223_846_793_005)
                    .wrapping_add(1_442_695_040_888_963_407);
                let player_index = (entropy as usize) % players_xp.len();

                let tree = &mut trees[tree_index];
                let previous_roll = tree.roll;
                let previous_health = tree.health;
                let previous_logs = tree.logs;
                let previous_fuel = tree.fuel;
                let previous_xp = players_xp[player_index];
                let previous_player_logs = players_logs[player_index];
                let previous_player_fuel = players_fuel[player_index];
                ...
                let next_fuel = previous_fuel.checked_sub(reward).unwrap();
                let next_xp = previous_xp.checked_add(reward).unwrap();
                let next_player_logs = previous_player_logs.checked_add(reward).unwrap();
                let next_player_fuel = previous_player_fuel.checked_add(reward).unwrap();
                ...
                ChopTransition {
                    ...
                    tree_fuel_before: previous_fuel,
                    tree_fuel_after: next_fuel,
                    player_logs_before: previous_player_logs,
                    player_logs_after: next_player_logs,
                    player_fuel_before: previous_player_fuel,
                    player_fuel_after: next_player_fuel,
                    player_value_before: 330,
                    player_value_after: 330,
                    tree_value_before: 1_980,
                    tree_value_after: 1_980,
                ...
                crate::player::PlayerChopTransition {
                    ...
                    state_fuel_before: previous_player_fuel,
                    state_fuel_after: next_player_fuel,
                    tree_markers_before: 1,
                    tree_markers_after: 1,
                    tree_logs_before: previous_logs,
                    tree_logs_after: next_logs,
                ...
                tree.logs = next_logs;
                tree.fuel = next_fuel;
                players_xp[player_index] = next_xp;
                players_logs[player_index] = next_player_logs;
                players_fuel[player_index] = next_player_fuel;
                attempts += 1;
                assert!(attempts < 20_000, "schedule {seed} did not terminate");
            }

            assert!(trees.iter().all(|tree| {
                tree.regrown && tree.health.value() == 0 && tree.logs == 0 && tree.fuel == 0
            }));
            assert!(players_xp
                .iter()
                .zip(players_logs)
                .zip(players_fuel)
                .all(|((xp, logs), fuel)| *xp == logs && *xp == fuel));
            assert_eq!(
                trees.iter().map(|tree| tree.logs).sum::<u64>() + players_logs.iter().sum::<u64>(),
                100
            );
            assert_eq!(
                trees.iter().map(|tree| tree.fuel).sum::<u64>() + players_fuel.iter().sum::<u64>(),
                100
            );
        }
    }
    ...
    #[test]
    fn scripts_commit_counter_fuel_and_health_shapes() {
        let tree = asset(1, 0);
        let log = asset(1, 1);
        let fuel = asset(1, 2);
        let chop = tree_covenant_script(tree, log, fuel, 330).unwrap();
        let regrow = tree_regrow_covenant_script(tree, log, fuel, 330).unwrap();
        let renewal = tree_renewal_covenant_script(tree, log, fuel).unwrap();
