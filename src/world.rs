...
pub(crate) const XP_FUEL_PER_TREE: u64 = 10;
...
impl WorldManifest {
    ...
    pub fn validate<C: Verification>(
        ...
        if self.map_width != MAP_WIDTH
            ...
            || self.xp_fuel_per_tree != XP_FUEL_PER_TREE
        ...
        let xp_fuel_asset = txbuild::parse_asset_id_pub(&self.xp_fuel_asset)
            .ok_or_else(|| anyhow!("invalid XP_FUEL asset ID in world manifest"))?;
        let genesis_txid =
        ...
        if tree_asset.txid != genesis_txid
            ...
            || xp_fuel_asset.txid != genesis_txid
            || tree_asset.group_index != 0
            || log_asset.group_index != 1
            || xp_fuel_asset.group_index != 2
        {
            return Err(anyhow!(
    ...
    pub async fn verify_indexed_assets(&self, rest: &crate::arkade::ArkadeRest) -> Result<()> {
        for (asset_id, label, maximum_supply, allow_zero) in [
            ...
            (
                self.xp_fuel_asset,
                "XP_FUEL",
                XP_FUEL_PER_TREE * TREE_STATES.len() as u64,
                true,
            ),
        ] {
            let details = rest
...
mod tests {
    ...
    fn manifest_round_trips_exactly_ten_trees() {
        ...
        assert_eq!(world.xp_fuel_asset.to_string(), parsed.xp_fuel_asset);
    ...
    fn manifest_rejects_changed_gameplay_policy() {
        ...
        for mutate in [
            ...
            |manifest: &mut WorldManifest| manifest.xp_fuel_per_tree += 1,
        ...
        let (_, _, _, mut manifest) = fixture();
        manifest.xp_fuel_asset = manifest.log_asset.clone();
        assert!(manifest.validate(&secp, &params, &emulator).is_err());
    }
}
