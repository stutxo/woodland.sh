...
//! XP_FUEL remain in that state; the numeric XP packet must equal its XP_FUEL
...
/// paired with one burned world XP_FUEL unit by the tree covenant.
...
pub struct PlayerChopTransition {
    ...
    pub state_fuel_before: u64,
    pub state_fuel_after: u64,
...
impl PlayerChopTransition {
    pub fn validate(self) -> Result<()> {
        ...
        if self.previous_state.xp.value() != self.state_fuel_before
            || self.next_state.xp.value() != self.state_fuel_after
            || self.state_fuel_after
                != self
                    .state_fuel_before
                    .checked_add(reward)
                    .ok_or_else(|| anyhow!("player XP_FUEL overflow"))?
        {
            return Err(anyhow!(
                "player XP must equal its conserved XP_FUEL balance"
            ));
        }
...
pub fn build_player_contract<C: Verification>(
    ...
    if [tree_asset, log_asset, xp_fuel_asset]
        ...
        != 3
    {
        return Err(anyhow!("TREE, LOG, and XP_FUEL assets must differ"));
    }
    let chop_arkade_script =
        player_chop_covenant_script(log_asset, xp_fuel_asset, dust_sats, tree_script)?;
    let renewal_arkade_script =
        player_renewal_covenant_script(log_asset, xp_fuel_asset, dust_sats)?;
...
/// self-send preserving P2TR, value, packets, PLAYER_ID, LOG, and XP_FUEL.
pub fn player_renewal_covenant_script(
    ...
    let builder = crate::tree::push_player_renewal_group_set(builder, log_asset, xp_fuel_asset);
...
fn canonical_player_state_assets<'a>(
    ...
    let fuel_entries = record
        .assets
        .iter()
        .filter(|asset| asset.asset_id == contract.xp_fuel_asset)
        .count();
    let canonical_assets = (1..=3).contains(&record.assets.len())
        ...
        && record.assets.iter().all(|asset| {
            asset.asset_id == player_asset
                ...
                || asset.asset_id == contract.xp_fuel_asset
        });
...
///
/// The player state owns its LOG and XP_FUEL inventory directly. The shared
/// tree covenant enforces the reciprocal transfers and deterministic roll.
///
...
/// enforces the reciprocal TREE/LOG/XP_FUEL and health deltas.
pub fn player_chop_covenant_script(
    ...
    if log_asset == xp_fuel_asset {
        return Err(anyhow!("LOG and XP_FUEL assets must differ"));
    }
    if dust_sats == 0 {
    ...
    let builder = crate::tree::push_transfer_group_shell(builder, xp_fuel_asset);

    let builder =
        crate::tree::push_canonical_player_asset_counts(builder, log_asset, xp_fuel_asset);

    // Numeric XP is exactly the conserved XP_FUEL held by player state.
    let builder = crate::tree::push_optional_input_asset_lookup(
        builder,
        PLAYER_STATE_INPUT_INDEX,
        xp_fuel_asset,
...
mod tests {
    ...
    #[test]
    fn successful_chop_moves_log_and_fuel_into_player_state() {
        let valid = transition(true);
        valid.validate().unwrap();

        let mut missing_fuel = valid;
        missing_fuel.state_fuel_after -= 1;
        assert!(missing_fuel.validate().is_err());
    ...
    fn player_contract_supports_owner_and_watchtower_renewal() {
        ...
        let record = crate::arkade::VtxoRecord {
            ...
            assets: vec![
                ...
                Asset {
                    asset_id: contract.xp_fuel_asset,
                    amount: 4,
                },
            ],
            created_at: Some(1),
