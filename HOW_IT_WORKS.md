...
before:
  ...
  tree:   health H, F XP, N LOG, one TREE, 1,980 sats

after:
  player: 1 PLAYER_ID, XP X+G, X+G XP, M+G LOG, 330 sats
  tree:   health H-G, F-G XP, N-G LOG, one TREE, 1,980 sats
```
...
XP balance deltas. Both require the same extension and anchor shape.
...
clock time. The second five hits exhaust LOG and XP; that stump is permanent.
