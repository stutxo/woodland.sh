...
permissionless activation, owner renewal, and fixed world LOG/XP_FUEL supply.
...
- one recursive VTXO per player, holding 330 sats, one PLAYER_ID, state packets,
  LOG, and XP_FUEL;
- one recursive VTXO per tree, holding TREE, remaining LOG/fuel, health, and
  roll;
- no global player reserve, PLAYER_TICKET, allocator, or registry.
...
XP_FUEL moves into player state rather than disappearing, so supply conservation
