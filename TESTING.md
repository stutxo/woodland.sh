...
2. production WASM and GitHub Pages artifact compilation for browser transport,
   storage, manifest, CSP, and static-route paths;
...
validity. TREE, LOG, XP_FUEL, and each PLAYER_ID have no control asset; a fresh
...
arkd/emulator, deploy a fresh schema 1 world, build the root-level Pages bundle,
run browser and renewal stages, then stop the stack.
...
artifact contains a manifest-specific CSP, `.nojekyll`, and an explicit 404;
the development server adds only local maintenance health and watcher
supervision.
...
- one genesis txid with TREE group 0, LOG group 1, and XP_FUEL group 2;
...
- no control asset;
- ten tree VTXOs with one TREE, ten LOG, ten XP_FUEL, health five, and
  1,980 sats;
...
- player state holds harvested LOG and earned XP_FUEL;
- `player XP == player XP_FUEL` after every transition;
- `sum(tree LOG) + player LOG = 100`;
- `sum(tree XP_FUEL) + player XP = 100`;
...
- missing or extra XP/fuel/LOG delta;
...
issues a distinct PLAYER_ID, activates independently, and chops disjoint trees
...
Tests do not prove public service availability, hidden randomness, geography,
unique humans, hardened browser custody, or future operator/emulator signer
retention.
