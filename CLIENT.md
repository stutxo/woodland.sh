...
- the same amount of XP_FUEL as LOG;
- fixed 1,980 sats;
...
PLAYER_ID, and optional LOG and XP_FUEL. Numeric XP must equal XP_FUEL. Identity
is:
...
XP_FUEL:   player X + tree F -> player X+G + tree F-G
```
...
one-unit PLAYER_ID; append LOG and XP_FUEL groups when present. Sign with the
...
Player count is unlimited, while LOG and XP_FUEL season supply is fixed. The
