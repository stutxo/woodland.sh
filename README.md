...
There is no allocator, invitation, registry, player cap, or extra transaction.
...
```text
player state: D sats + 1 PLAYER_ID + identity + position + XP + LOG + XP_FUEL
initial tree: 1 TREE + 10 LOG + 10 XP_FUEL + health 5 + fixed 6D (1,980 sats)
```

Numeric XP is not independently forgeable: it must equal the XP_FUEL held by
the recursive player state. A successful swing transfers one LOG and one
XP_FUEL from the selected tree into player state and increments XP by one. A
miss transfers nothing. Total LOG and XP_FUEL remain 100.
The current 100-fuel world cannot reach the level-10 chance bonus; higher
...
XP_FUEL. Together, the reciprocal Arkade covenants enforce the
...
- health, LOG, and XP_FUEL deltas equal to the same drop bit;
- player XP equals player-held XP_FUEL before and after the swing.
...
bundle and manifest; it receives no deployer, maintenance, rollover, or player
secrets. Tree maintenance runs as a separate native operator process.
...
builds `mutinynet/woodland-world.json` by default; set the repository variable
`WOODLAND_PAGES_MANIFEST` to another tracked manifest path when required.
...
Commit the public mainnet manifest, set the repository variable
`WOODLAND_PAGES_MANIFEST` to its path, and push `main`; the Pages jobs build and
deploy only after the full CI/regtest gate passes. Attach the Pages site to the
production custom domain in repository settings.
...
unique humans, Sybil resistance, a global leaderboard, or service availability.
