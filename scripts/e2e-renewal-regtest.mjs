...
function worldAssetTotals(records, treeAsset, logAsset, xpFuelAsset) {
  return records.reduce(
    (sum, record) => {
      for (const asset of record.assets || []) {
        if (asset.assetId === treeAsset) sum.trees += Number(asset.amount);
        if (asset.assetId === logAsset) sum.logs += Number(asset.amount);
        if (asset.assetId === xpFuelAsset) sum.fuel += Number(asset.amount);
      }
      return sum;
    },
    { trees: 0, logs: 0, fuel: 0 },
  );
}
...
async function main() {
  ...
  assert.equal(manifest.xpFuelPerTree, 10);
  ...
  const xpFuelAsset = manifest.xpFuelAsset;
  ...
  const { info: fuelAssetInfo } = await assetInfo(xpFuelAsset, 'XP_FUEL');
  assert.equal(logAssetInfo.controlAsset || '', '');
  assert.equal(fuelAssetInfo.controlAsset || '', '');
  ...
  const totalsBefore = worldAssetTotals(worldBefore, treeAsset, logAsset, xpFuelAsset);
  assert.equal(totalsBefore.trees, 10);
  assert.equal(totalsBefore.logs, totalsBefore.fuel);
  ...
  assert.equal(holdings.get(xpFuelAsset), previousHoldings.get(xpFuelAsset));
  ...
  const totals = worldAssetTotals(world, treeAsset, logAsset, xpFuelAsset);
  assert.deepEqual(
    totals,
    totalsBefore,
