...
function assertXpFuelAccounting(state, expected, label) {
  const total = state.seasonXpRemaining + state.playerXp;
  assert.equal(total, expected, `${label}: fuel plus earned XP is ${total}`);
}
...
async function main() {
  ...
  try {
    ...
    assert.ok(initial.state.trees.every((tree) => tree.xpFuelRemaining === 10));
    ...
    assert.ok(initial.state.xpFuelAsset);
    assert.equal(
      new Set([
        initial.state.treeAsset,
        initial.state.logAsset,
        initial.state.xpFuelAsset,
      ]).size,
      3,
    );
    assertLogSupply(initial.state, 100, 'initial world');
    assertXpFuelAccounting(initial.state, 100, 'initial world');
    ...
    assertXpFuelAccounting(deployed.state, 100, 'activated player');
    ...
    chopped = await assertInvalidAssetPacketRejected(
      '__WOODLAND_E2E_INVALID_FUEL_GROUP',
      firstTree.treeId,
      'a foreign group cannot replace XP_FUEL',
    );
    chopped = await assertInvalidAssetPacketRejected(
      '__WOODLAND_E2E_INVALID_LOG_FUEL_ORDER',
      firstTree.treeId,
      'LOG and XP_FUEL groups cannot trade positions',
    );
    for (const [mutation, label] of [
      ...
      ['wrong-fuel-delta', 'XP_FUEL burn inconsistent with the roll must be rejected'],
    ...
    while (hits < TARGET_HITS) {
      ...
      if (attempts === 6) {
        ...
        chopped = await assertChopMutationRejected(
          'wrong-fuel-delta',
          firstTree.treeId,
          'a successful chop without exactly one XP_FUEL burn must be rejected',
        );
      }
      ...
      assert.equal(
        current.xpFuelRemaining,
        beforeTree.xpFuelRemaining - Number(success),
      );
      ...
      assertXpFuelAccounting(chopped.state, 100, `swing ${attempts}`);
    ...
    assert.equal(stump.xpFuelRemaining, 5);
    ...
    assert.equal(regrownTree.xpFuelRemaining, 5);
    assertLogSupply(chopped.state, 100, 'regrown tree');
    assertXpFuelAccounting(chopped.state, 100, 'regrown tree');
    ...
    assertXpFuelAccounting(chopped.state, 100, 'post-regrowth reload');
    ...
    while (
      ...
      assert.equal(current.xpFuelRemaining, 5 - depletionHits);
      ...
      assertXpFuelAccounting(chopped.state, 100, `reserve depletion swing ${depletionAttempts}`);
    ...
    assert.equal(exhaustedTree.xpFuelRemaining, 0);
    ...
    assert.equal(permanentlyDepletedTree.xpFuelRemaining, 0);
    ...
    assert.match(chopped.mapHint, /exhausted its LOG and XP_FUEL reserve/);
    assert.equal(chopped.state.playerLogs, exhaustedLogs);
    assertLogSupply(chopped.state, 100, 'permanently depleted tree');
    assertXpFuelAccounting(chopped.state, 100, 'permanently depleted tree');
    ...
    assertXpFuelAccounting(chopped.state, 100, 'second tree chop');
    ...
    assertXpFuelAccounting(chopped.state, 100, 'final reload');
    ...
    console.log(JSON.stringify({
      ...
      xpFuelAsset: chopped.state.xpFuelAsset,
